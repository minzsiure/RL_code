...
class FQLAgent_Torch:
    ...
    def ensure_tensor(self, x):
        """Ensure the input is a torch.Tensor on the same device as the model."""
        if not torch.is_tensor(x):
            # Get device from the network's parameters.
            device = next(self.network.model.parameters()).device
            return torch.tensor(x, dtype=torch.float32, device=device)
        return x

    def critic_loss(self, batch, grad_params=None, rng=None):
        # Ensure all batch elements are tensors.
        batch['observations'] = self.ensure_tensor(batch['observations'])
        batch['actions'] = self.ensure_tensor(batch['actions'])
        batch['next_observations'] = self.ensure_tensor(batch['next_observations'])
        batch['rewards'] = self.ensure_tensor(batch['rewards'])
        batch['masks'] = self.ensure_tensor(batch['masks'])

        if rng is None:
            rng = torch.Generator(device=next(self.network.model.parameters()).device)
        ...
        q_ensemble = self.network.model(**{
            'observations': batch['observations'],
            'actions': batch['actions']
        }, name='critic')
        if self.config['q_agg'] == 'min':
            q = torch.min(q_ensemble, dim=0)[0]
        else:
            q = torch.mean(q_ensemble, dim=0)
    ...
    def actor_loss(self, batch, grad_params=None, rng=None):
        ...
        # BC flow loss
        x_0 = torch.randn(batch_size, action_dim, generator=rng, device=batch['actions'].device)
        x_1 = self.ensure_tensor(batch['actions'])
        t = torch.rand(batch_size, 1, generator=rng, device=batch['actions'].device)
        x_t = (1 - t) * x_0 + t * x_1
        vel = x_1 - x_0
        ...
        # distillation loss
        noises = torch.randn(batch_size, action_dim, generator=rng, device=batch['actions'].device)
        ...
        mse_val = F.mse_loss(actions, batch['actions'])
    ...
    def sample_actions(self, observations, seed=None, temperature=1.0):
        observations = self.ensure_tensor(observations)
        if seed is None:
            seed = torch.Generator(device=observations.device)
            seed.manual_seed(random.randint(0, 2**32 - 1))
        ...
        noises = torch.randn(noise_shape, generator=seed, device=observations.device)
